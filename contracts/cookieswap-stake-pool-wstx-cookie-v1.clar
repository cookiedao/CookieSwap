;; @contract Stake Pool - Stake wSTX-USDA LP tokens
...
(impl-trait .cookieswap-stake-pool-trait-v1.stake-pool-trait)
...
;; Constants
(define-constant POOL-TOKEN .cookie-swap-token-wstx-cookie)
...
(define-public (stake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .cookie-dao get-qualified-name-by-name "stake-registry"))) ERR-NOT-AUTHORIZED)
    ...
    (let (
      ...
      (try! (contract-call? .cookie-swap-token-wstx-cookie transfer amount staker (as-contract tx-sender) none))
...
(define-public (unstake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (let (
    ...
    (let (
      ...
      (try! (as-contract (contract-call? .cookie-swap-token-wstx-cookie transfer amount tx-sender staker none)))
...
(define-public (emergency-withdraw (registry-trait <stake-registry-trait>))
  (begin
    ...
    (asserts! (is-eq (contract-of registry-trait) (unwrap-panic (contract-call? .cookie-dao get-qualified-name-by-name "stake-registry"))) ERR-WRONG-REGISTRY)
    ...
    (let (
      ...
      ;; Transfer LP token back from this contract to the user
      (try! (as-contract (contract-call? .cookie-swap-token-wstx-cookie transfer stake-amount tx-sender staker none)))
    ...
    (let (
      ...
      (if (>= pending-rewards u1)
        (begin
          ...
          (try! (contract-call? .cookie-dao mint-token .cookie-token pending-rewards staker))
...
(define-public (calculate-cumm-reward-per-stake (registry-trait <stake-registry-trait>))
  (let (
    (rewards-per-block (unwrap-panic (contract-call? registry-trait get-rewards-per-block-for-pool .cookieswap-stake-pool-wstx-cookie-v1)))
...
(define-private (get-last-block-height (registry-trait <stake-registry-trait>))
  (let (
    (deactivated-block (unwrap-panic (contract-call? registry-trait get-pool-deactivated-block .cookieswap-stake-pool-wstx-cookie-v1)))
